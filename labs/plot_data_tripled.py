...
if __name__ == "__main__":
    ...
    axs = [fig.add_subplot(gs[i, j]) for j in range(3) for i in range(0, 5)]
    ...
    for DATABASE_NAME in DATABASE_NAMES:
        ...
        for column in df.columns:
            ...
            if (plot_idx + 1) % (num_subplots / 3) == 0:
            ...
            if SHARE_X:
                if (
                    plot_idx % (num_subplots / 3) == num_subplots / 3 - 1
                    and plot_idx != 0
                ):
                    ref_text = references[int(plot_idx // (num_subplots / 3))]
        ...
        index = int(shift * num_subplots / 3)
        for name in groups_names:
            ...
            for group in groups:
                ...
                if name not in group_colors:
                    ...
                    group_colors[name] = gcolors[int(index % (num_subplots / 3))]
                    index += 1

                gplot_idx = int(shift * num_subplots / 3)
                for column in df.columns:
                    if column in SKIP_COLUMNS:
                        continue
